...
class Spa extends EventEmitter {
  ...
  init() {
    ...
    .then(s => {
      ...
      if (isAuthError) {
        logError(`Failed to initialize client: ${err}. This looks like an authentication error (wrong ControlMySpa username/password?) - please check your credentials. Retrying in 30 minutes to avoid hammering the login endpoint.`);
        setTimeout(() => self.init(), 30 * 60 * 1000);
      } else {
        logError(`Failed to initialize client: ${err}. Retrying in 30 seconds...`);
        setTimeout(() => self.init(), 30 * 1000);
      }
    });
  }
  ...
  registerListeners() {
    ...
    self.on('initialized', () => {
      ...
      self.scheduleTokenRefresh(self.tokenRefreshInterval());
    });
  ...
  // Schemalägger nästa tokenförnyelse. Ersätter det tidigare setInterval-
  // upplägget (som bara räknade ut intervallet EN gång, vid start, och sedan
  // aldrig kunde justera sig) med en självjusterande kedja av setTimeout -
  // varje körning räknar ut nästa väntetid på nytt, utifrån vad vi faktiskt
  // vet just då.
  scheduleTokenRefresh(delayMs) {
    let self = this;
    if (self._tokenRefreshTimer) {
      clearTimeout(self._tokenRefreshTimer);
    }
    self._tokenRefreshTimer = setTimeout(() => {
      self.refreshToken();
    }, delayMs);
  }
  ...
  async refreshToken() {
    ...
    try {
      await self.client.refreshAccessToken();
      logDebug(`Access token refreshed via refresh_token flow. Expires in ${self.client.tokenData.expiresIn}s`);
      self.emit('token_updated');
    } catch (err) {
      logError(`Failed to refresh access token via refresh_token flow: ${err}. Falling back to full login...`);

      try {
        await self.client.login();
        logDebug('Fallback: full login (email/password) succeeded');
        self.emit('token_updated');
      } catch (loginErr) {
        logError(`Fallback full login also failed: ${loginErr}. Will try again at next scheduled refresh.`);
      }
    }

    // Schemalägg nästa förnyelse utifrån vad vi nu faktiskt vet (eller, om
    // allt ovan misslyckades, samma försiktiga fallback-gissning som innan).
    self.scheduleTokenRefresh(self.tokenRefreshInterval());
  }

  tokenRefreshInterval() {
    let self = this;
    // Om vi har ett verkligt, känt expiresIn (från en lyckad refresh_token-
    // förnyelse via Azure B2C) - använd det exakta värdet med en liten
    // säkerhetsmarginal (5 minuter). Vi vet då precis hur länge token håller,
    // ingen anledning att gissa.
    //
    // Annars (första gången, direkt efter en vanlig lösenordsinloggning via
    // /auth/login - den endpointen returnerar inget expires_in alls) -
    // falla tillbaka på en försiktig gissning: 23 timmar. Detta är bara ett
    // skyddsnät för den allra första cykeln; redan efter första lyckade
    // refreshAccessToken()-anropet ersätts gissningen av det verkliga värdet.
    let expiresIn = self.client.tokenData ? self.client.tokenData.expiresIn : undefined;
    if (expiresIn) {
      return Math.max((expiresIn - 5 * 60) * 1000, 60 * 1000);
    }
  ...
  getDeviceInfo() {
    ...
    let info = {
      serialNumber: spa.spaSerialNumber || spa.serialNumber,
      productName: 'N/A',
      model: 'N/A',
      dealerName: spa.dealer?.name || 'N/A',
      registrationDate: 'N/A',
      manufacturedDate: 'N/A',
      buildNumber: spa.systemInfo?.buildNumber || 'N/A'
    };
    return info;
  }
  ...
  getRangeLowTemp() {
    ...
    return this.useCelsius() ? ((parseFloat(temp) - 32) * (5 / 9)).toFixed(1) : temp;
  }
  ...
  // Used only by the standalone "Toggle Heater Mode" button entity.
  // Refreshes state from the cloud first so the toggle direction is based
  // on the freshest data we can get, instead of a potentially stale cache.
  // If the refresh itself fails, we abort rather than guess the wrong direction.
  toggleHeaterMode() {
    let self = this;
    self.client.getSpa().then(() => {
      let curMode = this.getHeaterMode();
      let newMode = curMode === 'READY' ? 'REST' : 'READY';
    ...
    }).catch(err => logError(`Failed to refresh spa state before toggling heater mode, aborting toggle to avoid guessing the wrong direction: ${err}`));
  }

  // Absolute, non-toggle way to set heater mode. Used by the HA climate
  // entity's mode_command_topic ("heat"/"off" -> "READY"/"REST").
  setHeaterMode(desiredMode) {
    let self = this;
    let curMode = this.getHeaterMode();
    self.client.setHeaterMode(desiredMode).then(res => {
      logDebug(`Set heater mode to ${desiredMode} complete: ${JSON.stringify(res)}`)
      if (res && desiredMode !== curMode) {
        this.emit('status_updated')
      } else {
        setTimeout(() => self.updateSpa(), self.refreshAfterUpdate);
      }
    }).catch(err => logError(`Failed to set heater mode to ${desiredMode}: ${err}`));
  }
  ...
  // Used only by the standalone "Toggle Temperature Range" button entity.
  // Same fresh-state-first safeguard as toggleHeaterMode().
  toggleTempRange() {
    let self = this;
    self.client.getSpa().then(() => {
      let curMode = this.getTempRange();
      let newMode = curMode === 'HIGH' ? 'LOW' : 'HIGH';
    ...
    }).catch(err => logError(`Failed to refresh spa state before toggling temp range, aborting toggle to avoid guessing the wrong direction: ${err}`));
  }  
