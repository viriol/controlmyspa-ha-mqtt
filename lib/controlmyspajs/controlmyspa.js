...
// DNS cache + fallback
//
// Balboa's/ControlMySpa's nameservers (ns29/ns30.worldnic.com) have proven
// to be intermittently broken ("lame delegation" / SERVFAIL). A regular
// axios/Node request does a fresh DNS lookup for every single request and
// has no cache of its own, so it's extremely vulnerable to this.
//
// This function stores the last successful lookup per hostname. If a fresh
// lookup fails, the last known-good result is used instead of letting the
// whole request crash with getaddrinfo/EAI_AGAIN.
// ---------------------------------------------------------------------------
const dnsCache = new Map(); // hostname -> { address, family, timestamp }

// Seed value: last known, stable IP for iot.controlmyspa.com (unchanged
// throughout months of troubleshooting). This makes the fallback work
// already on the very first attempt after a restart, without needing HA's
// own DNS lookup to succeed even once first - which has proven unreliable
// regardless of what we do on our end (see the ha dns options/restart
// troubleshooting). Automatically overwritten by a real, fresh lookup as
// soon as one succeeds.
dnsCache.set('iot.controlmyspa.com', {
  address: '13.83.102.3',
  family: 4,
  timestamp: Date.now() // seed timestamp set at module load, overwritten by the first successful lookup
});

function cachedLookup (hostname, options, callback) {
  // dns.lookup can be called as (hostname, callback) or (hostname, options, callback)
  ...
  dns.lookup(hostname, options, (err, address, family) => {
    ...
    // No age limit - as long as we've ever successfully looked up this
    // hostname, the last known-good IP is used until a fresh lookup
    // succeeds and overwrites it. Overwritten automatically on every
    // successful attempt.
...
// Shared axios instance used by every call in this file, so the DNS
// cache/fallback applies everywhere without each individual call needing
// to specify httpsAgent manually.
...
class ControlMySpa {
  ...
  // Refreshes the access token via Azure B2C's OAuth2 token endpoint
  // directly, using the refresh_token we already got from /auth/login.
  // Requires no password and doesn't load Balboa's own (historically
  // fragile) login endpoint.
  //
  // IMPORTANT: Azure B2C rotates the refresh_token on every use - the old
  // one becomes invalid as soon as a new one has been obtained. We MUST
  // therefore save the new refresh_token from the response, not reuse the
  // old one.
  //
  // The client_id below is the same "azp" value we found in the decoded
  // access token JWT. If Balboa ever changes this, the request will return
  // a clear AADB2C error message (Azure B2C is unusually good at
  // descriptive errors), which makes it much easier to debug than the
  // silent 404s we got used to.
  async refreshAccessToken () {
    ...
    if (req.status === 200) {
      // NOTE: the response field names (access_token/refresh_token/expires_in)
      // are snake_case (raw Azure B2C standard), unlike /auth/login which
      // returns camelCase (accessToken/refreshToken). We normalize here to
      // the same camelCase structure the rest of the code expects.
      this.tokenData = {
  ...
  // Primary: /user-agreements/current (the original integration's endpoint)
  // Fallback: /web/user-management/profile (the one we found via web sniffing)
  async getProfile () {
  ...
  // Primary: /spas/owned (the original integration's endpoint)
  ...
  // Primary: /spas/<id>/dashboard (the original integration's endpoint, OLD field names:
  //   isOnline, isCelsius, rangeLimits, time as an "HH:MM" string, isPanelLocked)
  // Fallback: /web/spas/<id>/current-state (NEW field names:
  //   online, celsius, setupParams, hour/minute separately, panelLock)
  //
  // spa.js consistently expects the NEW field names (online, celsius,
  // setupParams, hour/minute, panelLock) - see isOnline()/useCelsius()/
  // getRangeLowTemp()/getTime()/isPanelLocked() in spa.js. That means it's
  // the PRIMARY response (dashboard) that needs to be normalized to the
  // new format when used - NOT the fallback response, which is already in
  // the right format.
  async getSpa () {
    ...
    try {
      ...
      // Normalize OLD field names (dashboard) to the NEW field names spa.js expects
      ...
      // Nothing to normalize - current-state already delivers the new field names spa.js wants.
